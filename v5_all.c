...
//Handle streambuffer do zero-crossing
StreamBufferHandle_t buffer_freq;
            ...
            if (parse_ret == ESP_OK) {
                for (int i = 0; i < num_parsed_samples; i++) {
                    if (parsed_data[i].valid) {
                        ...
                        // Estimação de frequência - Zero-Crossing.
                        if (ultima_amostra <= 0.0f && amostra_atual > 0.0f && contador_amostras > 250) {
                            frequencia_estimada = 20000.0f / (float)contador_amostras;
                            xStreamBufferSend(buffer_freq, &frequencia_estimada, sizeof(frequencia_estimada), 0);
                            contador_amostras = 0;
                        }
                        ...
                        if (sample_idx >= SAMPLES_PER_PERIOD) {
                            ...
                            for(int j=0; j < n; j++)
                            {
                                // note que estou realizando a equação abaixo duas vezes.
...
void display(void *pvParameters)
{
    ...
    while(1)
    {
        char form[20] = "TensaoRMS:"; // Precisa ser reatualizado a cada volta do loop
        // Recebe os dados do buffer
        xStreamBufferReceive(buffer_oled, &dados, sizeof(dados), portMAX_DELAY);
        xStreamBufferReceive(buffer_freq, &freq, sizeof(freq), portMAX_DELAY);
        ESP_LOGI(TAG, "Freq calculada: %.2f Hz | RMS: %.2f V", freq, dados);
...
void app_main(void)
{
    ...
    buffer_oled = xStreamBufferCreate(16,1); 
    buffer_freq = xStreamBufferCreate(16,1);
