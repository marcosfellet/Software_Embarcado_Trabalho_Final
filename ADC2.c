...
void vtaskProcessamento(void *pvparameters)
{
    ...
    uint32_t raw = 0;
    int tensao_mv = 0;
    ESP_LOGI(TAG, "task processamento");

    while (1)
    {
        //uint32_t total = 0;
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        while (xStreamBufferReceive(buffer, &raw, sizeof(raw), portMAX_DELAY) == sizeof(raw))
        {
            adc_cali_raw_to_voltage(cali_handle, raw, &tensao_mv);
            //ESP_LOGI(TAG, "Tensão: %d mV", tensao_mv);
            //total++;
            //float tensao_mv_float = (float)tensao_mv;
            //comunicacao_serial(&tensao_mv_float);
        }
...
void app_main(void)
{
    ...
    xStreamBufferSetTriggerLevel(buffer, MAX_SAMPLES*sizeof(int));
    xTaskCreate(vtaskADC, "Task ADC", 8192, NULL, 4, NULL); // por eu não precisar controlar ela eu não preciso passar um handle para ela. ela roda em loop.
